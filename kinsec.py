...
async def call(ctx, target: discord.User, *, message: str):
    ...
    await ctx.send(
        f"Process complete. Delivery status: "
...
async def on_message(message: discord.Message):
    ...
    if (
        message.author.bot
        or is_whitelisted(message.guild, message.author)
    ):
...
async def on_audit_log_entry_create(
    ...
    # BOT ADD
        ...
        if (
            unauthorized_bot
            and getattr(unauthorized_bot, "bot", False)
        ):
...
async def on_ready():
    ...
    print(
        f"Kinsec Active. Logged in as: {bot.user}"
    )
